...
module.exports = {
    ...
    async execute(client, message, args, Hyperz, config) {
        ...
        if (config["tickets_config"].enabled !== true) return message.channel.send("This module is disabled, please ask the owner to enable it!").then(msg => msg.delete({ timeout: 9000 }))
        ...
        message.channel.send(`Please confirm that you wish to archive this ticket.`).then(balls => {
            ...
            balls.awaitReactions(johncena, { max: 1, time: ms("25m") }).then(collected => {
                ...
                if (react23847.emoji.name === '❌') {
                    return message.channel.send(`Cancelling ticket archive process.`).then(msg => msg.delete({ timeout: 10000 }));
                }
            })
        }).catch(console.error);
    } else {
